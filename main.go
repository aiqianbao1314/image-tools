...
func runSkopeo(args ...string) error {
	// 1. 获取二进制文件内容
	...
	// 2. 将其写入临时目录以便执行
	tempDir := os.TempDir()
	tempSkopeo := filepath.Join(tempDir, "skopeo-internal")
	
	// 写入并赋予执行权限 (0755)
	err = os.WriteFile(tempSkopeo, data, 0755)
	if err != nil {
		return fmt.Errorf("写入临时文件失败: %v", err)
	}
	// 程序退出时清理临时文件
	defer os.Remove(tempSkopeo)

	// 3. 使用 os/exec 调用该二进制
	cmd := exec.Command(tempSkopeo, args...)
